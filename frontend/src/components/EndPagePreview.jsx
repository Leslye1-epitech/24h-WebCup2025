...
export default function EndPagePreview({ formData, themes }) {
    ...
    const getReasonText = (formData) => {
        if (formData.reason === 'job') return "a quitté son emploi";
        if (formData.reason === 'relationship') return "a mis fin à une relation";
        if (formData.reason === 'project') return "a quitté un projet";
        if (formData.reason === 'group') return "a quitté un groupe";
        if (formData.reason === 'other') return formData.customReason || "a dit au revoir";
        return "a tourné une page";
    };
